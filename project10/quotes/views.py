...
class TagDetailView(View):
    ...
    def get(self, request, name:str):
        db = get_mongodb()
        author = list(db.quoters.find({"tags":name}))
                 
        return render(request, self.template_name, context = {"tag":name, "quotes":quotes})
    

class AddQuoteView(CreateView):
    template_name = "quotes/add_quote.html"
    from_class = QuoteForm
    success_url = reverse_lazy("quotes:quote_list")
